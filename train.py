...
@click.option('--precond',       help='Preconditioning & loss function', metavar='vp|ve|edm|cdm',       type=click.Choice(['vp', 've', 'edm', 'cdm']), default='cdm', show_default=True)
...
def main(**kwargs):
    ...
    if opts.precond == 'vp':
        ...
        c.network_kwargs.class_name = 'training.networks.EDMPrecond'
        c.loss_kwargs.class_name = 'training.loss.MartingaleLoss'
        c.loss_kwargs.martingale_lambda = opts.martingale_lambda
        c.loss_kwargs.S_churn = opts.s_churn
        c.loss_kwargs.S_min = opts.s_min
        c.loss_kwargs.S_max = opts.s_max
        c.loss_kwargs.S_noise = opts.s_noise
        c.loss_kwargs.num_steps = opts.martingale_steps
    else:
    ...
    if opts.augment:
        ...
        c.augment_kwargs.update(xflip=1e8, yflip=1, scale=1, rotate_frac=1, aniso=1, translate_frac=1, 
            masking_prob=opts.ambient_masking)
