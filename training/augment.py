...
class AugmentPipe:
    def __init__(self, p=1,
        ...
        # AmbientGAN augmentations
        self.masking_prob = float(masking_prob)             # Probability to delete a pixel
    ...
    def __call__(self, images):
        ...
        if self.masking_prob > 0:
            # mask is 1 to the positions that survive, zero-otherwise
            mask = (torch.rand_like(images[:, 0, :, :]) > self.masking_prob).to(torch.float32)
            images = images * torch.unsqueeze(mask, 1)
